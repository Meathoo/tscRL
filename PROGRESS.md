...
**意外的收穫**：那些退化成常數 meta 的 arm 構成了一個先前沒有的對照組。16x3 上拆解：

| | 16x3 zero-shot | 相對前一列 |
|---|---:|---:|
| `learned`（隨機碼） | 308.24 | — |
| 常數 meta（`struct4` / `lanecount2`） | 246–255 | **−53～−62** |
| `struct`（12 維，邊界指示） | 226.67 | −20～−29 |

**`struct` 勝過 `learned` 的 81.6 裡，約三分之二是「不要餵隨機碼」，只有三分之一是結構資訊。**
`learned` 的 per-intersection 表在遷移時被 shape filter 跳過、換成隨機初始化的碼；常數 meta 就足以打敗它。
**論文若拿 `struct` vs `learned` 當賣點而沒有常數 meta 對照組，會把兩件事的功勞都記在結構條件化上。**
已加入 `--agent_embedding_mode constant`（`0a91eb7`）把這個對照做成有意的 arm，2026-08-27 起在 `.237` 上跑。
...
- **執行中（2026-08-27）**：`constmeta` 4x4 來源訓練 3 seeds（`.237`），完成後接 zero-shot，
  用來把 (l) 的三分之二／三分之一拆解變成有意的對照而非副產物。
