...
**(p) capacity 在 Ingolstadt21 上——負面結果，唯一成功的移植不能推廣（2026-08-29，`.232`）**
...
|---|---:|---:|---:|
| `struct`（5 seeds） | **203.75 ± 6.89** | **220.55 ± 6.26** | **226.47 ± 9.12** |
| `structcap`（3 seeds） | 216.72 ± 4.32 | 235.50 ± 19.52 | 243.87 ± 12.18 |

**方向與 16x3 完全相反**（那裡 capacity 早 75–100 個 episode 收斂、seed 變異小 23 倍）。
差距是 best +13.0、tail10 +17.4。**不是完全分離**：`struct` 的 seed 4 是五個裡最差的
（best 215.96、tail10 240.96），落在 `structcap` 的區間內；另外四個 seed 全勝過三個
`structcap` seed。所以方向明確、幅度中等。

推測機制（未驗證）：Ingolstadt21 的車道容量全距是 33 倍（1.1–36.1 輛），
`obs_capacity_clip: 1.5` 之下，1.1 輛的短車道只要有 2 台車就飽和，
**等於把那些車道的觀測壓成常數**。16x3 的容量差距只有 3.5 倍，沒有這個問題。
`obs_capacity_clip` 從來沒有掃過，這是唯一便宜的翻盤實驗。

**結論：`obs_norm_mode: capacity` 只在同質、車道長度差距溫和的網上有用。
不要把它設成預設，也不要用它重跑 Ingolstadt 矩陣。**

**(q) `constmeta` 在 Ingolstadt21 上——結構特徵的內容在異質網上值 98 秒（2026-08-29，`.232`）**

(l) 的結論是「`struct` 勝過 `learned` 的 ~84 全部來自移除逐路口索引表，
結構特徵的內容貢獻約為零」。**那個結論只在格狀網上成立**——那裡 12 個特徵有 10 個是常數，
`constmeta` 打平 12 維契約（224.52 vs 226.67）是自洽的。
Ingolstadt21 是特徵真的會變的網，這一格先前沒做。做了之後：

| | best | last | tail10 |
|---|---:|---:|---:|
| **`struct`**（hypernet + 結構 meta，5 seeds） | **203.75 ± 6.89** | **220.55 ± 6.26** | **226.47 ± 9.12** |
| `mappo`（無 hypernet，共用 MLP，3 seeds） | 230.21 ± 8.44 | 270.34 ± 10.04 | 263.00 ± 6.97 |
| `constmeta`（hypernet + 常數 meta，3 seeds） | 234.40 ± 17.59 | 303.65 ± 45.96 | 324.70 ± 46.64 |

三個 arm 的 reward、obs、PPO 超參、預算、critic 全部一致，差別只在權重怎麼產生。

**排序是 `struct` < `mappo` < `constmeta`，而這個排序才是重點。**
`constmeta` 不只輸給 `struct` 98.2 秒，還輸給**根本沒有 hypernetwork** 的共用 MLP 61.7 秒。
所以贏的不是「hypernetwork 這個參數化」——給它一個常數 code，它比不用還糟。
**贏的是條件化的內容，而且只在內容真的有變化的網上。**

`constmeta` 的失效模式是**守不住**，不是學不到：seed 0 的 best 是 214.10（可用），
但 ep150→200→245 走 272→261→265；seed 2 走 314→468→355。
訓練 q_loss 比 `struct` 高 20 倍（0.00021–0.00024 對 0.00001）。
逐路口生成同一份權重，等於用一個高度冗餘的參數化去表達一個共用策略。

**對論文的意涵（取代 (l) 的表述）**：兩句話都要講，因為它們在不同路網上成立——
> 在**同質**路網上，條件化的內容不值錢（`constmeta` 打平 12 維契約），
> `struct` 勝過 `learned` 是因為 meta 路徑沒有索引表可以被形狀過濾丟掉。
> 在**異質**路網上，條件化的內容值 98 秒；沒有內容的 hypernetwork 比沒有 hypernetwork 更差。
...
**(s) phase head 的單網基準（2026-08-30，`.232`）——前提通過，但有三件事要分開講**

(o-3) 設的前提是：movement encoder 本身在單網上就有代價（`mestruct` 對 `struct`），
phase head 疊上去必須先站得住。3 seeds × 250 ep，同機器：

| arm | best | last | tail10 | tail10 throughput | 完成率 |
|---|---:|---:|---:|---:|---:|
| `struct`（無 encoder，5 seeds） | 203.75 ± 6.89 | 220.55 ± 6.26 | 226.47 ± 9.12 | 3976 | **92.84%** |
| `mestruct`（encoder，舊 head） | 209.74 ± 8.83 | 220.58 ± 15.72 | 246.59 ± 30.03 | 3760 | 87.80% |
| **`phstruct`（encoder + phase head）** | **199.53 ± 1.55** | **212.37 ± 1.11** | **217.41 ± 10.20** | 3868 | 90.32% |
| `phlearned`（同上但用索引表） | 209.97 ± 3.24 | 223.58 ± 7.98 | 226.43 ± 7.32 | 3864 | 90.22% |

（總需求 4283 台，departure 窗 57600–61200.8 與模擬窗完全吻合。）

**(s-1) 前提通過，而且 encoder 的傷害根本不是 encoder 造成的。**
`phstruct` 勝過 `mestruct` 在**兩個指標上同時成立**——tail10 217.41 對 246.59，
完成率 90.32% 對 87.80%。完成率同向改善，所以這不是 survivorship。
機制說得通：encoder 產生的是 movement 層級的 token，而舊的 action-indexed head
把它們 pool 掉才輸出固定寬度的 logit——**等於先建構再丟棄**。
phase head 直接以原本的形式消費它們。所以 (o-3) 那個警告的正確版本是
「**movement encoder 不該單獨開**」，不是「encoder 有代價」。

**(s-2) `phstruct` 勝過 `struct` 這件事「不成立」，別拿去用。**
`last` 差 8.2 秒而且三個 seed 完全不重疊（phstruct 211.3–213.5，struct 214.9–230.6），
看起來很漂亮——但 `struct` 的完成率高 2.5 個百分點，**多完成 108 台車**。
要讓這 108 台解釋掉整個 8.2 秒的差距，它們的平均旅行時間只需要是 513 秒，
也就是網路平均的 2.3 倍。**在壅塞網的尾端，那完全是正常值。**
所以現有數據無法區分「phase head 控制得更好」與「phase head 少放了 108 台最慢的車」。
要分辨得報 (m) 說的那組指標，不能只看 travel time。

**(s-3) 但變異數的改善是真的，survivorship 解釋不了它。**
`last` 的 sd 從 `struct` 的 6.26 掉到 **1.11**（三個 seed 落在 2.2 秒之內），
`best` 的 sd 從 6.89 掉到 1.55。完成率高低不影響 seed 之間的離散程度。
**這是整個研究裡最穩定的一組 run。**

**(s-4) 結構條件化在 phase head 之下仍然贏，但幅度縮到六分之一。**
`phstruct` 對 `phlearned`：完成率 90.32% 對 90.22%（實質相同，所以這個比較乾淨），
tail10 217.41 對 226.43，差 **9.0 秒**。
對照沒有 head 時的 `struct` 對 `learned`：226.47 對 280.70，差 **54.2 秒**。

> **這一點對論文的賣點是負面的，必須自己講出來。**
> phase head 本身就是結構感知的——它用「這個相位放行哪些 movement」來評分，
> 那正是結構特徵想編碼的資訊的一部分。所以 **head 取代掉了條件化向量原本的大部分功能**。
> 正確的表述是：條件化的價值取決於 actor 有多少結構資訊是自己拿得到的；
> actor 越是結構感知，額外餵它結構向量的邊際價值越低。

**下一步**：前提通過（(s-1)），跨異質路網的遷移可以開跑。但 (s-2) 意味著
**任何用 phase head 得到的遷移數字都必須同時報完成率**，否則無法與非 head 的基準相比。

**(t) 三件事：phstruct 補到 5 seeds、cologne3 終於有尺、`cityflow4x4_hetero` 是負面結果（2026-08-30，`.232`）**

**(t-1) `phstruct` 5 seeds，並補上 travel time 以外的指標。**
seed 3/4 補完後與 5-seed 的 `struct` 同規模（tail10 均值）：

| | travel | queue | delay | reward | throughput | wall-clock |
|---|---:|---:|---:|---:|---:|---:|
| `struct`（5） | 226.47 ± 9.12 | 1.207 ± 0.116 | 0.1324 ± 0.0041 | −3.329 ± 0.418 | 3976.9 ± 81.4 | 12231 ± 265 s |
| `phstruct`（5） | **215.44 ± 7.82** | **0.657 ± 0.058** | **0.0934 ± 0.0031** | **−1.689 ± 0.299** | 3866.0 ± 16.1 | 15938 ± 558 s |

逐 seed 的 `last`：`struct` = 214.90 / 216.57 / 222.20 / 218.43 / 230.63，
`phstruct` = 212.25 / 211.32 / 213.54 / 216.32 / 208.59——**5 個裡 4 個完全在 `struct` 之下**，
只有 phstruct seed3（216.32）與 struct seed0（214.90）交叉。`best` 與 `tail10` 不分離。

> ⚠️ **queue 與 delay 不能拿來反駁 (s-2) 的 survivorship 質疑。**
> 它們看起來是壓倒性的（queue −46%、delay −29%，都完全分離），但**網路裡車少的時候
> 這三個數字會一起下降**，而 `phstruct` 的完成率正是低 2.5 個百分點。
> 四個指標裡有三個都吃同一個偏誤，(s-2) 的結論不變：**要分辨得報完成率配對的比較。**

站得住的兩件事：**變異數**（`best` sd 1.24 對 6.89，`last` sd 2.85 對 6.26）
和**成本**（wall-clock 貴 30%，4.4 小時對 3.4 小時）。

**(t-2) cologne3 的零樣本終於有尺，而且量出一個分離。**
(o-3) 之後做的 `Ingolstadt21 → cologne3` 只有一個裸數字，沒有上下界所以無法解讀。
補上：下界＝同架構不載入任何 checkpoint 的評估（10 seeds，取中位數，因為 3 seeds 時
`168.1` 在三種架構都出現，隨機策略會塌到同一個固定相位）；上界＝在 cologne3 上從零訓練 250 ep。

| 架構 | 隨機初始化（10 seeds 中位數） | cologne3 從零訓練（tail10） | Ingolstadt21 → cologne3 zero-shot |
|---|---:|---:|---|
| `struct` | 430.4 | **64.12 ± 3.66** | 載入失敗（08-28 起 exit 1）|
| `mestruct` | 436.9 | 84.56 ± 1.27 | **65.02 ± 9.09** [61.5, 58.2, 75.3] |
| `phstruct` | 219.6 | **61.09 ± 1.67** | 130.37 ± 111.29 [68.0, 64.3, **258.9**] |
| `phlearned` | — | — | 180.30 ± 104.26 [262.1, 62.9, 215.9] |

**`zs_mestruct` 打敗同架構在 cologne3 上自己訓練 250 個 episode**（65.02 對 84.56，best 也是
65.02 對 77.33）。以最好的從零訓練臂（`phstruct` 61.09）當上界，zero-shot 走完 **98.9%** 的距離
——與 (b) 的 4x4→16x3（97.1%）同級，但這次是異質→異質、跨路網。

**但 `phstruct` 的遷移比 `mestruct` 差得多**（130.37 對 65.02，sd 差 12 倍，三個 seed 有一個
炸到 258.9）。也就是**(s-1) 裡單網最好的架構不是最會遷移的架構**——同一組 checkpoint，
在來源網上贏 29 秒，搬到目標網輸 65 秒。這兩個性質會分開，論文裡要當成兩件事寫。

> ⚠️ **cologne3 可能撐不起遷移宣稱。** `zsrand_phstruct` 最好的 seed 是 **75.1**，
> 而 `zs_mestruct` 的平均是 65.02——**隨機初始化的下界與遷移結果重疊**。
> 只有 3 個受控路口，隨機策略有時候就是不差。這是量出來的，不是猜的。

**(t-3) `cityflow4x4_hetero`——設計來破「異質≡SUMO」混淆的實驗，出來是 null。**
動機：所有 null 都在 CityFlow 合成格網，所有 win 都在 SUMO 真實路網，
**異質性與模擬器在現有資料裡完全對齊**，審稿人一定會問。`cityflow4x4_hetero` 同引擎、
同城市（hangzhou gudang）、同車流檔，12 維裡有 8 維不是常數（格網只有 2 維）。
4 臂 × 3 seeds × 250 ep，同機器：

| arm | best | tail10 |
|---|---:|---:|
| `learned` | **281.24 ± 0.50** | **282.05 ± 0.09** |
| `constmeta` | 282.94 ± 0.19 | 283.69 ± 0.13 |
| `struct` | 283.38 ± 1.04 | 284.97 ± 0.77 |
| `mappo`（無 hypernet） | 421.23 ± 55.58 | 464.25 ± 37.08 |

**Δ(struct − learned) = −2.9，方向是反的；`constmeta` 與另外兩臂只差 1–3 秒。**
條件化的內容在這張網上完全沒有作用。

兩張網的排序完全不同，而這才是收穫：

| | 條件化的內容值多少 | hypernetwork 參數化值多少 |
|---|---:|---:|
| Ingolstadt21 | **98.2 s**（`struct` 226 vs `constmeta` 325）| **−61.7 s**（`constmeta` 比 `mappo` 還糟）|
| cityflow4x4_hetero | **≈ 0**（三臂差 1–3 s）| **+180 s**（`mappo` 464 vs 三臂 ~283）|

> **參數化與內容是可以分開的兩件事，哪一半在贏取決於路網。** 這比 (q) 的表述精確。

**為什麼是 null——「有幾維不是常數」是錯的 x 軸。** 真正的差別是**幅度**：

| 路網 | in_lane 範圍 | CV | phase_count | Δ(struct − learned) |
|---|---|---:|---|---:|
| cityflow4x4 / 16x3 / 7x28 | 12 固定 | 0 | 8 固定 | ≈ 0 |
| **cityflow4x4_hetero** | **9–12**（mean 11.25）| **0.115** | **8 固定** | **−2.9** |
| sumo_atlanta1x5 | 5–8（mean 6.60）| 0.182 | **2–5** | 執行中 |
| ingolstadt21 | 4–14（mean 7.43）| **0.30** | **2–4** | +54.2 |

`4x4_hetero` 雖然 8 維不是常數，但 `in_lane` 只在 9–12（16 個受控路口裡 12 個是 12、4 個是 9），
`phase_count` 仍固定為 8。CV 差 2.6 倍。這也與 `lanecount2`（只用 in/out lane count）
是 Ingolstadt 全部臂裡 tail10 最好的一格（214.70）對得起來：**是車道數的離散程度在起作用。**

**誠實的結論：混淆沒有被破解。** 現在確定的只有「CityFlow + 輕微異質 → Δ=0」，
這同時相容於「需要更大的異質幅度」與「其實是真實路網的別的因素」。範圍縮小，兩解未分。

**(t-4) 為了分開這兩解而加的旋鈕：`--structural_shrink`（`84bb018`）。**
沒有任何一對現有路網能單獨變動「幅度」。`--structural_shrink s` 把每一維朝
**該網自己的平均**收縮：`x → mean + s·(x − mean)`，順序與中心不變，只有離散程度改變。
在同一張網上掃它，就是在其他條件全部固定下變動幅度。

- `s = 1.0` **整段跳過**而不是乘以 1（float32 下 `mean + 1.0*(x-mean)` 不是逐位元的 `x`）。
  逐位元驗證：cityflow4x4 `struct` 一個 episode 完全重現
  （q_loss 0.0031771005579718835、travel 1592.0815、throughput 817）。
- 收縮後的特徵**依賴載入的路網**，正是這個模組其餘部分拒絕做的事，所以 `spec_id()` 標記
  （`structural_v1+shrink0.6:...`），收縮過的 checkpoint 無法被完整契約的 run 載入。
  **只能當同網 ablation，永遠不要當遷移來源。**
- 156 個測試通過，其中 7 個是新的。
...
- **(s)/(t) 已完成**，數字在 §6.2。(o-3) 的「B4 暫停」已過期：前提通過（(s-1)），
  但 (s-2)/(t-1) 的完成率問題與 (t-2) 的「單網最好 ≠ 最會遷移」都必須跟著一起報。
- **2026-08-30 起在 `.232` 上執行中**（派工腳本在機器的 `tmp/p012/`，不進版控）：
  1. `n2_shrink` — Ingolstadt21 的 `--structural_shrink` 掃描，`shr06`/`shr038`/`shr02`/`shr00`
     × 3 seeds × 250 ep。`shrink=1.0` 就是既有的 5-seed `struct`（tail10 226.47）；
     **0.38 = 0.115/0.30，即讓 Ingolstadt 穿上 `4x4_hetero` 的離散程度**；
     `shrink=0.0` 讓所有路口相同，應該落在 `constmeta`（324.70）附近，兼作旋鈕的一致性檢查。
     若 tail10 隨 shrink 單調上升、且 0.38 附近就吃掉對 `learned`（280.70）的優勢，
     (t-3) 的「幅度」解釋成立，混淆才算破解。
  2. `n1_atlanta` — `sumo_atlanta1x5` 的同一組 4 臂 × 3 seeds。CV 0.182 落在
     `4x4_hetero`（0.115）與 Ingolstadt（0.30）之間，且 `phase_count` 是 2–5（`4x4_hetero` 固定 8），
     是唯一能替 (t-3) 那張表補中間點的網。5 個號誌，很便宜。
  3. `n3_seeds` — `mestruct` / `phlearned` 在 Ingolstadt 補 seed 3/4，然後把
     `→ cologne3` 的 zero-shot 全部重跑到 5 seeds。(t-2) 的分離目前各只有 3 seeds，
     而 `phstruct` 的 111 標準差是單一 seed 撐起來的。
