...
> **三輪跑完之後（2026-09-04）的一句話已經不是上面這句了：**
> A 被自己的 kill test 擋下、B 是負面結果、C 準確度是 null。
> 真正立得住的是它們**合起來**量到的一條軸——
> **重要的是「有效策略種數」，21 個路口大約 8 種剛好；1 種差 90 秒，21 種差 52 秒。**
> 分組的**內容**不重要，分組的**數量**很重要。詳見 §0.4。
...
| 方案 | 狀態 | 依據 |
|---|---|---|
| **A** Relational | **已停止**，未建置 | kill test 未通過（§4.4.1）。`scripts/edge_contract_stats.py` |
| **B** Regime Mixer | **已實作、實驗完成——負面結果**，輸 baseline 30–60 秒 | §0.3。`agent/mixer.py` |
| **C** K-Prototype | **已實作、實驗完成——準確度 null、算力 win** | §0.2。`agent/hypernetwork.py` |

**三個方案都沒有提升準確度。** 這條線真正的產出是三輪合起來量到的
**有效策略種數**那條軸（§0.4）——它比任何一個原方案的賣點乾淨，
而且同時解釋了 `constmeta`／`learned`／`struct` 三個既有結果。
...
> **2026-09-02 停電**：`.237` 之後無法開機，它上面跑到一半的 12 個 run 全部作廢
> （跨機器不可並列，所以不能只補沒跑完的臂）。`sumo1x21` 整批在 `.232` 重跑，
> 24 個 run（8 個 tag × 3 seeds）於 2026-09-04 全部完成。
> `.232` 自己的 `struct500` / `lanecount2500` 在停電前已跑滿 500 ep，未受損。

---

## 0.2 方案 C 的實驗結果（2026-09-04）

### 0.2.1 兩個端點都通過，`k0` 是逐位元重現

這是整個設計裡「可檢查」的部分，先報它：

- **`k0`（K=0，不分解）三個 seed 的 tail10 = 224.6 / 225.8 / 225.4，
  與既有 `struct` 的前三個 seed 完全相同的數字。**
  `hyper_prototypes: 0` 的 bit-identity 因此在真正的 250-episode run 上成立，
  不只是在 1-episode smoke 上。
- **`k1`（K=1）= 315.08 ± 20.98，對既有 `constmeta` 324.70 ± 46.64**——
  同樣的失敗、同樣的量級。gate 診斷顯示 K=1 的 perplexity 恰好 1.00、max share 1.000，
  也就是它在數值上確實塌成單一權重集合。

軸校準完成，中間的點才可以讀。

### 0.2.2 K 曲線（`sumo1x21`，250 ep × 3 seeds，`.232` 同機同批）

| K | tail10 | best | throughput | 完成率 | actor perplexity |
|---:|---|---|---:|---:|---|
| 0（不分解） | **225.25 ± 0.65** | 200.82 ± 1.27 | 3963 | 92.54% | — |
| 1 | **315.08 ± 20.98** | 223.94 ± 7.03 | 3505 | 81.84% | 1.00 |
| 2 | 249.15 ± 10.84 | 210.82 ± 2.54 | 3920 | 91.53% | 1.9 |
| 4 | 249.10 ± 26.86 | 212.36 ± 10.14 | 3950 | 92.21% | 2.2–2.5 |
| 8 | **223.28 ± 16.18** | 208.15 ± 3.97 | 4040 | 94.32% | 4.0–4.8 |
| 16 | 224.82 ± 10.24 | 207.19 ± 2.69 | 4025 | 93.98% | 3.0–8.2 |
| 21 (=N) | 226.56 ± 11.99 | 207.25 ± 1.64 | 4008 | 93.58% | 4.8–12.6 |
| **8，凍結 gate** | **224.92 ± 13.16** | 210.90 ± 2.50 | 4022 | 93.91% | 7.7–7.8 |

（總需求 4283 台。`perplexity` = exp(entropy)，即這個 arm 實際表現得像幾個 prototype，
由 checkpoint 裡的 gate 佔用率算出，`scripts/prototype_diagnostics.py`。）

`cityflow4x4_hetero`（local，250 ep × 3 seeds）：`k0` 283.30 ± 1.19、`k8` 284.47 ± 0.41，
Δ = +1.17 且 seed 區間重疊——**null，與建構上的預測一致**（格網的 meta 近乎常數）。

### 0.2.3 三個讀法

**(1) K=8 沒有贏過 K=0。** 223.28 ± 16.18 對 225.25 ± 0.65，差 1.97 秒，
而 `k8` 的三個 seed 橫跨 206.8–239.2。**mean 上是 null。**
值得記一筆的是完成率同向（94.32% 對 92.54%），所以這 2 秒不是 (s-2) 那種 survivorship，
只是幅度太小；但方向對不代表有效果。

**(2) 凍結 gate 的對照把機制主張打掉了。**
`k8f`（固定隨機分組，gate 凍在隨機初始化）224.92 ± 13.16、完成率 93.91%，
與 `k8` 統計上無法區分。**學到的分組一文不值。**
更說明問題的是：隨機 gate 的 actor perplexity（7.7–7.8）比學到的（4.0–4.8）**更平均**——
學到的 gate 確實會集中到少數 prototype，但集中沒有換到任何東西。

**(3) 這條曲線給出一個取代 (q) 的結論。** 把三個點排在一起：

| 各路口的權重 | tail10 |
|---|---:|
| 全部相同（K=1 / `constmeta`） | 315.08 |
| 任意分成 8 組（`k8f`，隨機且固定） | 224.92 |
| 依結構連續區分（`k0` / `struct`） | 225.25 |

> **`constmeta` 的失敗不是「沒有內容」，是 rank 1。**
> 給它一個**毫無意義**的分組就完全修好了（315 → 225）。

> ⚠️ **這一段初稿寫成「分幾組、分組有沒有道理，都不重要」，後半對、前半錯。**
> 「分組有沒有道理」確實不重要（`k8f` 打平 `k0`），
> 但「**分幾組**」非常重要——§0.4 的 `frz` 臂把這一點推翻了：
> 每個路口一組（21 組）比 8 組差 52 秒。正確的表述在 §0.4。

這比 (q) 的「條件化的內容在異質網上值 98 秒」精確，而且**部分推翻它**：
(q) 用 `constmeta` 當「有 hypernetwork 但沒有內容」的代表，
但 `constmeta` 同時也是「所有路口共用一組生成權重」。這兩件事在 (q) 裡是綁在一起的，
`k8f` 把它們拆開後，掉出來的是後者。
...
| 原本的主張 | 結果 |
|---|---|
| 準確度追平 `learned`／`struct` | ✅ K ≥ 8 追平（但沒有超過） |
| 生成算力省 6.1× | ✅ 不受影響（449.7 M → 74.3 M MACs） |
| 參數量幾乎不變 | ✅ checkpoint 320.1 MB 對 318.5 MB（+0.5%） |
| K 曲線是一個正面貢獻 | ⚠️ 改成**負面／機制結果**：曲線在 K≥8 之後平掉，且分組內容無關 |
| 「結構相似的路口該共用策略」 | ❌ 被 `k8f` 否定 |

**結論：C 是準確度上的 null、算力上的 win、機制上的負面結果。**
它值得寫，但要寫成「為什麼 `constmeta` 會失敗」的答案，
不要寫成「一個更好的架構」。

---

## 0.3 方案 B 的實驗結果（2026-09-04）——負面結果

`sumo1x21`，250 ep × 3 seeds，`.232` 同機同批，`hyperlight_ppo`（local critic，
因為 mixer 就是 centralization）。

| arm | 說明 | tail10 | 完成率 | per-seed tail10 |
|---|---|---|---:|---|
| `b1` | regime 碼固定（等於不看狀態） | **252.91 ± 1.54** | 89.48% | 252.1 254.7 252.0 |
| `b8c` | K=8，**不**量化（連續碼） | 260.40 ± 14.62 | 88.74% | 252.0 277.3 252.0 |
| `bu` | uniform mixer（w≡1），B 的對照 | 264.72 ± 11.05 | 85.83% | 270.8 252.0 271.3 |
| **`b8`** | **完整方案 B**（K=8 量化 regime） | **282.75 ± 52.34** | 82.72% | 253.1 **343.2** 252.0 |
| — | baseline `struct` / `k0`（非 mixer） | **225.25 ± 0.65** | 92.54% | |

**兩個都是負面的：**

1. **整個 B 家族輸給 baseline 30–60 秒**，而且完成率同時較低（82.7–89.5% 對 92.5%），
   所以不是 survivorship 造成的假象——是真的比較差。
2. **在 B 內部，最好的是 `b1`——那個把 regime 碼固定、等於完全不看車流狀態的版本。**
   真正的方案 B（`b8`）最差，而且 seed 1 跑飛到 343.2（另外兩個 seed 是 253.1 / 252.0）。

**所以 (h) 的教訓在 critic 上一樣成立。** B 的整個立論是「狀態條件化不是方向錯，是注入點錯」，
把它從 policy 移到 credit assignment、再加上量化就能救。**這個賭注輸了**：
移了注入點、加了量化，狀態條件化仍然沒有幫助，而且在這裡是**有害**的
（`b8` 對 `b1` 差 29.8 秒）。量化也沒有救它——`b8c`（不量化）反而比 `b8`（量化）好。

**⚠️ 一個還沒補完的 caveat。** B 相對於既有表格同時改了兩件事：
目標函數變成聯合 return，以及 pooled centralized critic 換成 local critic。
所以現在還分不出「輸的是聯合目標」還是「輸的是少了 pooled critic」。
`bn`（local critic + 原本的 per-agent 目標 + 完全沒有 mixer）2026-09-04 13:42 派出，
3 個 run，**執行中**。補完之後這一節的第 1 點才算完整。

---

## 0.4 三輪合起來的主結論：**有效策略種數**

這是這條線真正的產出，而且它比 A／B／C 任何一個原本的賣點都乾淨——
它用一條軸同時解釋了 `constmeta`、`learned`、`struct` 三個既有結果，
而那三個在既有文件裡是三條各自獨立的觀察。

### 0.4.1 資料

全部在 `sumo1x21`、`.232`、250 ep × 3–5 seeds：

| 有幾種不同的權重 | arm | tail10 |
|---|---|---:|
| **1 種**（全部路口相同） | `k1` / `constmeta` | **315.08** |
| 2 種 | `k2` | 249.15 |
| 4 種 | `k4` | 249.10 |
| **8 種（隨機亂分，凍結）** | `k8f` | **224.92** |
| **8 種（依結構分）** | `k8` | **223.28** |
| 16 種 | `k16` | 224.82 |
| 21 種（結構 gate，實測有效 4.8–12.6） | `k21` | 226.56 |
| 連續（結構特徵，平滑） | `k0` / `struct` | **225.25** |
| **21 種（每路口一個，隨機凍結）** | `frz` | **277.40** |
| **21 種（每路口一個，可訓練）** | `lrn` / `learned` | **293.18** |

（`lrn` 跑出 261.8 / 310.4 / 307.4，與既有 `aw` 臂**逐位元相同**——runner 的又一次校準。）

### 0.4.2 結論

> **中間有最佳點。太少不行，太多也不行。**
> 21 個受控路口大約 8 種策略剛好；1 種差 90 秒，21 種差 52 秒。

兩個分開的敘述，一個成立一個不成立：

- **分組的「內容」不重要。** `k8f`（隨機亂分）打平 `k8`（依結構分）打平 `k0`（連續結構）。
  這是 §0.2.3 的結論，仍然成立。
- **分組的「數量」很重要。** `frz` 與 `k8f` 只差 cardinality（都是凍結、都是隨機、
  都沒有內容），8 組是 224.92、21 組是 277.40。**差 52.5 秒。**

### 0.4.3 為什麼太多也不行

一個 episode 的資料由全部 21 個路口共享。切成 21 份，每個路口的策略只分到 1/21；
切成 8 份，每份有 2–3 個路口的資料。這是統計效率，不是表達力。

### 0.4.4 這解釋了三個既有結果

| 既有觀察 | 在這條軸上的位置 |
|---|---|
| `constmeta` 比不用 hypernetwork 還差（(q)） | 有效種數 = 1，太少 |
| `learned` 在異質網輸 `struct` 54 秒（(o-2)） | 有效種數 = N，太多 |
| `struct` 一直贏 | 結構特徵是**平滑**的映射，結構像的路口生出像的權重，**有效種數自動落在中間**——實測 `k21` 的 perplexity 只有 4.8–12.6 而不是 21，直接量到了這件事 |

**`struct` 的價值因此需要重新表述**：不是「結構特徵編碼了有用的資訊」
（`k8f` 證明內容可以完全無意義），而是「結構特徵提供了一個**平滑**的條件化，
它把有效策略種數壓在合適的範圍」。任何達成同樣壓縮的東西都一樣好。

### 0.4.5 這推翻／修正了什麼

- **(l)／(t-2) 對 `learned` 的解釋要修正。** 既有說法是索引表不能遷移所以 `learned` 差。
  但 `frz` 有**完全相同的索引表**、完全相同的 cardinality，只是碼不訓練——
  它 277.40 對 `learned` 293.18，**兩者重疊、沒有分開**。
  所以「碼要不要被學」不是原因，**cardinality 才是**。
  索引表確實是遷移的障礙（那部分不變），但它**在單網訓練上也差**，
  而那個原因是有效種數太多，跟遷移無關。
- **§0.2.3 的初稿表述要修正**，見該節的警告框。

### 0.4.6 還沒做、但這條軸指出來的

這條軸是量出來的，不是設計出來的，所以它沒有被任何一個 arm **直接**驗證過。
最便宜的直接驗證是在**別的路網**上重跑 cardinality 掃描，看最佳點是不是隨 N 移動：

- 若 `cityflow7x28`（N=196）的最佳點落在遠大於 8 的地方 → 支持「每組要有足夠資料」的機制
- 若仍然落在 ~8 → 最佳點與 N 無關，機制另有其他

這是一個 `k2/k4/k8/k16/k32/k64` × 3 seeds 的掃描；7x28 很貴（本機 ~80 h／run），
所以應該先在 `cityflow16x3`（N=48）上做。
