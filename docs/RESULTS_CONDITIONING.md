...
唯一有價值的條件化訊號是「路口的結構」，而且只在結構真的會變的路網（Ingolstadt21）上有價值；
把條件化改成跟著車流狀態走則三個路網都沒有好處。

**真正有效的不是條件化，是觀測正規化**：把每條車道的計數改成除以它自己的儲存容量
（移植自 BRSC-MAPPO 的 `incoming_occupancy`），在 16x3 上讓收斂早 75–100 個 episode、
seed 變異小 23 倍（§2.4）。
...
| 7x28 | 同質、重度壅塞（queue ~14） | 1360.13 ± 34.58 | 1390.93 ± 19.14 | 差 30.8 秒，**seed 重疊** |

**7x28 的中途讀數會誤導，這裡記下來避免重蹈**：ep≈175 時是 1307.22 ± 13.17 vs
1385.09 ± 30.77，差 78 秒且三個 seed 完全不重疊，看起來像決定性的傷害；
跑滿 250 episode 後差距縮到 30.8 秒且 seed 重疊
（struct 最差 1380.8 > structdyn 最好 1369.4）。
**結論仍是「沒有好處」，但「有害」講得太滿。**

（這批 7x28 因 `.232` 於 2026-08-23 05:35 自動重開機而從 ep25 續跑過，
兩臂處理相同，但絕對值不宜與其他 7x28 資料並列。）
...
### 2.4 觀測正規化（capacity vs fixed）— 唯一明確有效的移植

移植自 BRSC-MAPPO 的 `incoming_occupancy`：每條車道的計數除以**它自己的儲存容量**
（長度 / 7.5m）而非全域常數 `vehicle_max=50`。實作見 `transfer/observation.py`。

在 **16x3**（網內車道 100m 與 350m，差 3.5 倍）上，兩臂都用 structural condition：

| | last (mean ± std) | per-seed |
|---|---:|---|
| struct（fixed） | 211.73 ± 28.17 | 224.7 / 231.1 / 179.4 |
| **structcap（capacity）** | **180.42 ± 1.22** | 179.4 / 181.8 / 180.1 |

**但「好 31 秒」不是正確的讀法**——看收斂曲線就知道差別在**速度**不在終點：

| | ep50 | ep100 | ep150 | ep200 | ep245 |
|---|---:|---:|---:|---:|---:|
| struct seed0 | 1492.0 | 1468.8 | 964.4 | 309.8 | 224.7 |
| struct seed1 | 1549.4 | 1261.9 | 852.8 | 358.2 | 231.1 |
| struct seed2 | 1456.6 | 595.5 | 920.8 | 186.2 | 179.4 |
| structcap seed0 | 1381.0 | 915.4 | **243.9** | 181.6 | 179.4 |
| structcap seed1 | 1232.4 | **300.9** | 245.6 | 248.5 | 181.8 |
| structcap seed2 | 1420.1 | 604.9 | **249.8** | 217.8 | 180.1 |

ep150 時 structcap 已經在 ~245，struct 還在 850–965；struct 在 ep245 仍在下降
（309.8 → 224.7），只是 250 個 episode 沒跑完。兩者的漸近值看起來一樣（~180），
**capacity 大約早 75–100 個 episode 到達，而且 seed 變異小 23 倍**（±1.22 vs ±28.17）。

機制說得通：`fixed` 模式下 policy 必須自己學會「同樣的 0.2 在 100m 車道和 350m
車道意義不同」，capacity 正規化把這件事直接做掉。

（注意 `struct` 這一臂是 16x3 上的新 run，與 §4.2 的 `aw`/`awrf` 在不同機器上，
不可直接並列比較；`struct` vs `structcap` 同機同批，才是乾淨的比較。）
